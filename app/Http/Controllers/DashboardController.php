...
class DashboardController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        if ($user->company_id && $user->can('view-company-customers')) {
            $leadsMetrics = $this->leadsMetrics($request, $user->company_id);
        } elseif ($user->can('view-all-customers')) {
            $leadsMetrics = $this->leadsMetrics($request, $request->integer('company_id') ?: null);
            $leadsMetrics['companies'] = Company::where('is_active', true)
                ->orderBy('name')
                ->get(['id', 'name']);
        }
